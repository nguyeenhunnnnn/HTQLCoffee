...
namespace HTQLCoffee.View
{
    public partial class frmHDNhap : Form
    {
        ...
        // Tạo một lớp đại diện cho sản phẩm đã chọn
        ...
        private void HienThiComboBox()
        {
            ...
            comboboxNguyenLieu.DataSource = danhSachNguyenLieu;
            comboboxNguyenLieu.DisplayMember = "TenNguyenLieu";
            comboboxNguyenLieu.ValueMember = "NguyenLieuID";
            ...
            List<NhaCungCapModel> dsNCC = _nhaCungCapController.LayDanhSachNhaCungCap();
            comboboxNCC.DataSource = dsNCC;
            comboboxNCC.DisplayMember = "TenNhacungcap";
            comboboxNCC.ValueMember = "NhacungcapID";
        }
        private void addNL_Click(object sender, EventArgs e)
        {
            string productName = comboboxNguyenLieu.Text;
            float price;
            int quantity;
            if (comboboxNguyenLieu.SelectedValue == null)
            {
                MessageBox.Show("Vui lòng chọn một sản phẩm từ danh sách.", "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
            int productID = (int)comboboxNguyenLieu.SelectedValue;
        ...
        {
            if (comboboxNhanVien.SelectedIndex == -1 || comboboxNCC.SelectedIndex == -1 || selectedProducts.Count == 0)
            {
            ...
            int nhacungcapID = (int)comboboxNCC.SelectedValue;
            ...
            chitietPhieunhapDetails.Columns.Add("PK_iNguyenlieuID", typeof(int));
            chitietPhieunhapDetails.Columns.Add("fSoluong", typeof(float));
            chitietPhieunhapDetails.Columns.Add("fDongia", typeof(float));
            ...
            phieuNhap.NhacungcapID = nhacungcapID;
            phieuNhap.Ngaylap = ngaylap;
            _phieuNhapController.ThemMoiPhieuNhap(phieuNhap, chitietPhieunhapDetails);
